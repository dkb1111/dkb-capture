...
function startMeter(){
  ...
  meter=setInterval(()=>{
    ...
    $('meterFill').style.width=
      Math.min(100,Math.round(rms*360))+'%';
...
async function sendPending(){
  const url=
    localStorage.getItem('dkb_backend_url')||
    $('backendUrl').value.trim();

  if(!url){
    statusEl.textContent='BACKEND NOT SET';
    return;
  }
...
function confirmSaved(id,url,attempt){
  const cb=
    'DKB_CONFIRM_'+Date.now()+'_'+attempt;
  ...
  window[cb]=data=>{
    ...
    if(data?.found){
      finishSaveSuccess(data);

    }else if(attempt<10){
      confirmTimer=
        setTimeout(
          ()=>confirmSaved(id,url,attempt+1),
          900
        );
    }
  };
...
function finishReturnToFresh(){
  ...
  // Always clear the Notes cache after a successful save.
  // This forces NOTES to reload the latest notes from the backend.
  notesCache=[];
...
function loadNotes(){
  ...
  list.innerHTML=
    '<div class="card"><b>Loading notes…</b></div>';

  if(!url){
    list.innerHTML=
      '<div class="card">Backend not set.</div>';
    return;
  }
...
function renderNotes(items){
  ...
  if(!items.length){
    list.innerHTML=
      '<div class="card">No active notes.</div>';
  ...
  items.forEach(m=>{
    ...
    chk.checked=
      selected.has(m.matterId);

    chk.onchange=()=>{
      if(chk.checked){
        selected.add(m.matterId);
      }else{
        selected.delete(m.matterId);
      }
