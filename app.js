...
afterCard=$('afterCard'), player=$('player'), note=$('note'),
backendUrl=$('backendUrl'), syncForm=$('syncForm'), payloadField=$('payloadField');
...
async function startRecording(){
  try{
    ...
    chunks=[];qualitySamples=[];blob=null;
...
function stopRecording(){if(mr?.state==='recording')mr.stop()}
...
async function finishRecording(){
  ...
  qualityNote.textContent='Recording stored safely on this device.';
...
$('deleteBtn').onclick=async()=>{
  ...
  qualityNote.textContent='Ready for the next capture.';
};

$('syncBtn').onclick=async()=>{if(currentCapture)await syncCapture(currentCapture)};
...
async function syncCapture(c){
  ...
  if(!url){
    statusEl.textContent='SAVED ON DEVICE — BACKEND NOT SET';
    return;
  }
...
function checkConfirmation(captureId,attempt){
  ...
  window[callback]=async result=>{
    if(result?.ok&&result?.found){
      ...
      cleanup();
      if(attempt<8){
        confirmationTimer=setTimeout(()=>checkConfirmation(captureId,attempt+1),1200);
      }else{
        statusEl.textContent='SENT — NOT YET CONFIRMED';
      }
    }
  };
